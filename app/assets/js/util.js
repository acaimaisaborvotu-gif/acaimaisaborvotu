...
// DDDs que existem no Brasil. Fora desta lista, o numero e inventado.
const DDDS = new Set([11,12,13,14,15,16,17,18,19,21,22,24,27,28,31,32,33,34,35,37,38,
  41,42,43,44,45,46,47,48,49,51,53,54,55,61,62,63,64,65,66,67,68,69,
  71,73,74,75,77,79,81,82,83,84,85,86,87,88,89,91,92,93,94,95,96,97,98,99]);

// Telefone valido. Antes so contava digitos, entao "(00) 00000-0000" passava e a loja
// mandou entregador atras de pedido trote (26/08/2026). Agora confere o basico que todo
// numero brasileiro de verdade tem. NAO garante que o numero exista: pra isso so codigo
// por SMS/WhatsApp. Serve pra barrar o inventado na hora.
export function phoneValido(tel) {
  const d = semDDI(String(tel ?? '').replace(/\D/g, ''));
  if (d.length !== 10 && d.length !== 11) return false;
  if (!DDDS.has(Number(d.slice(0, 2)))) return false;
  const num = d.slice(2);                      // numero sem o DDD
  if (d.length === 11 && num[0] !== '9') return false;   // celular sempre comeca com 9
  if (d.length === 10 && !/^[2-5]/.test(num)) return false; // fixo comeca de 2 a 5
  if (/^(\d)\1+$/.test(num)) return false;     // 99999-9999, 00000-0000...
  return true;
}
